...
/* ----------------------------------------------------------------
 * TestimonialReel — Custom native implementation
 * Counter-rotating scroll reel + per-character text rise.
 *
 * This is a project-native recreation of the desired review/testimonial
 * reel UI & behavior (no external component generators or 21st.dev magic).
 * Geometry, masking, auto-advance, keyboard, pause, staggered chars,
 * and exit transitions are preserved exactly for visual parity.
 *
 * Stability fixes applied:
 * - Moving tiles use translate3d + backface-visibility for crisp corners.
 * - Border visuals on animated portrait cells use stable inset box-shadow
 *   (avoids subpixel border dance + blur + mask interactions during slide).
 * ---------------------------------------------------------------- */
...
export interface TestimonialReelProps {
  testimonials: TestimonialReelItem[];
...
/* Blurred placeholder cell — stable under motion (no border+blur combo) */
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl bg-gradient-to-b from-secondary to-card shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_0_0_1px_rgba(11,11,12,0.12),inset_0_2px_0_rgba(255,255,255,1)] dark:shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.06)]"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(225,6,0,0) 32%, rgb(225,6,0) 41%, rgb(255,120,110) 47%, rgba(255,90,80,0.57) 54%, rgba(255,90,80,0) 65%)",
        }}
      />
    </div>
  );
...
export function TestimonialReel({
  ...
  /* Auto-advance — ping-pongs between ends so it never does a long
   * rewind, and re-arms on every index change. Paused on hover/focus
   * and when reduced motion is preferred. */
  React.useEffect(() => {
    if (!autoPlayMs || count <= 1 || paused) return;
    const id = setTimeout(() => {
      let dir = autoDir.current;
  ...
  }, [index, autoPlayMs, count, paused, paginate]);
